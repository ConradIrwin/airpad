...
- (id)initWithAirbrakeData:(NSData *)data error:(NSError **)outError {
    // HACK: airbrake.io's API sometimes returns "invalid" XML, with tag names that are numeric.
    // to get round that, we just do a find-and-replace on the document to insert an x- before every such tag.
    // Using Latin1 as an 8-bit safe ASCII equivalent encoding so that I can run a regex search on the document.
    // TODO: should find out what happens if we get something with the wrong character encoding.
    NSMutableString *str = [[NSMutableString alloc] initWithData: data encoding: NSISOLatin1StringEncoding];
    [str replaceOccurrencesOfString:@"<(/?)(?=[0-9])" withString:@"<$1x-" options:NSRegularExpressionSearch range: NSMakeRange(0, [str length])];
    return [self initWithData: [str dataUsingEncoding: NSISOLatin1StringEncoding] error:outError];
}
